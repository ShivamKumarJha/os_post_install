@echo off
TITLE Windows 7 Post Installer - Created by Shivam Kumar Jha.
color 71
cls
echo.
echo Please wait while Windows 7 configures your system...
echo ... the configuration proccess may take up to 30 minutes, Windows 7 will restart automatically after the installation.
echo.
...
echo.
echo Net Frame Work 4.5
%windir%\Setup\Apps\dotnetfx45_full_x86_x64.exe /q /norestart
...
regedit /s /f "%windir%\Setup\Apps\Windows7Tweaks.reg"
REG ADD "HKCR\Directory\Background\shell\Clear Clipboard" /v "Position" /d "Top" /f
REG ADD "HKCR\Directory\Background\shell\Clear Clipboard\Command" /d "CMD.EXE /C ECHO OFF | CLIP" /f
regedit /s /f %windir%\Setup\Apps\DriverMax\ISOBUSTERREG.reg
regedit /s /f %windir%\Setup\Apps\Disable_Open-File_Security_Warning.reg
echo MSXML 6
...
echo Lammer Context Menu
%windir%\Setup\Apps\Lam.exe
...
%windir%\Setup\Apps\VC_redist.x64.exe /passive /quiet /install /norestart
echo Winrar 4.00
%windir%\Setup\Apps\WinRAR\winrar-64Bit-400.exe /s
echo 7-Zip
%windir%\Setup\Apps\7z1512-x64.msi /qb
echo Adobe Reader
...
%windir%\Setup\Apps\googlechromestandaloneenterprise64.msi /qb /norestart 
echo Hash Tab
...
echo VLC
%windir%\Setup\Apps\vlc-2.2.1-win64.exe /S
echo Windows 7 Manager
%windir%\Setup\Apps\Windows7Manager.exe
echo Webcam Viewer
%windir%\Setup\Apps\WV-SFX.exe
echo Virtual Box 5.0.10, click YES if promted to install driver.
%windir%\Setup\Apps\Vbox\VirtualBox-5.0.10-r104061-MultiArch_amd64.msi /qb
echo DFX11 Audio Enchancer, click YES if promted to install driver.
%windir%\Setup\Apps\DFX\dfx11Setup.exe /S /L /q /s /silent /qb /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /SP-
%windir%\Setup\Apps\DFX\Crack-x64.exe
